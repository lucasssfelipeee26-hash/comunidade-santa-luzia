...
function fail(message) { console.error(`[motion-beta12] ${message}`); process.exit(1) }
...
if (config.versionName !== "2.0.0-beta.12" || config.versionCode !== 20012) fail(`Configuração Beta 12 esperada, encontrado ${config.versionName}/code${config.versionCode}.`)
...
const jsRequired = {
  ...
  "android-constancia-luz-beta11.js": ["2.0.0-beta.11", "Constância de Luz", "POINTS_PER_DAY = 2", "MAX_DAYS = 7", "maximoSemanal: 14"],
  "android-report-bridge-beta11.js": ["2.0.0-beta.11", "patchMyFormation", "patchFormationBatch", "patchAdministrative", "patchDelayModeration"],
  "android-motion-parity-beta11.js": ["2.0.0-beta.12", "motionClockCompatibilityBeta12", "sl-b11-live-clock"],
...
const parityText = read(path.join(motionDir, "android-motion-parity-beta11.js"))
for (const forbidden of ["function trophyMarkup", "slB11CupFloat", "normalizeTrophy"]) if (parityText.includes(forbidden)) fail(`Troféu Beta 11 legado ainda ativo: ${forbidden}`)
...
if (!capConfig.includes(`SantaLuziaMotionBeta/${config.versionName}`)) fail("User-Agent Beta 12 ausente.")
