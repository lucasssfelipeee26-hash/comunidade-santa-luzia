...
for (const [file, label] of [[required.motion10, "runtime de sincronização"], [required.nativeFetch, "ponte de fetch nativa"], [required.domain, "regras de domínio"], [required.quiz, "Quiz Litúrgico offline"], [required.navigation, "navegação local"]]) {
...
for (const marker of ["reportar_atraso", "minha-presenca", "caminho-da-luz", "whatajong", "optimisticAdminQuiz", "optimisticTheme"]) if (!domain.includes(marker)) throw new Error(`Domínio Beta 10 sem marcador: ${marker}`)
...
console.log("Pré-validação Beta 10: frontend original, SyncHttp, navegação local, filas, Quiz Litúrgico e jogos presentes; nenhuma segunda interface offline.")
