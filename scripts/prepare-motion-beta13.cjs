...
function fail(message) {
  console.error(`[motion-beta13] ${message}`)
  process.exit(1)
}
function read(file) {
...
if (config.versionName !== "2.0.0-beta.13" || config.versionCode !== 20013) fail(`Configuração Beta 13 esperada, encontrado ${config.versionName}/code${config.versionCode}.`)
...
for (const marker of [
  ...
  "data-escala-history-enabled",
]) if (!localText.includes(marker)) fail(`Bundle local sem marcador Beta 13: ${marker}`)
...
if (/"server"\s*:/.test(capConfig)) fail("Motion Beta 13 não pode conter server.url no Capacitor.")
if (!capConfig.includes(`SantaLuziaMotionBeta/${config.versionName}`)) fail("User-Agent Beta 13 ausente.")
if (!capConfig.includes("SantaLuziaOriginalUIOffline/2")) fail("Identidade offline original ausente.")
if (!capConfig.includes("SantaLuziaWindowsBeta/0.1.0-beta.19")) fail("Referência visual Windows Beta 19 ausente.")

const main = read(path.join(root, "android", "app", "src", "main", "java", "br", "com", "comunidadesantaluzia", "app", "MainActivity.java"))
for (const marker of ["SyncHttpPlugin.class", "OfflineStorePlugin.class", "CaminhoDaLuzPlugin.class", "WhatajongPlugin.class", "LOAD_DEFAULT"]) if (!main.includes(marker)) fail(`MainActivity sem marcador: ${marker}`)
...
const offlineStore = read(path.join(root, "android", "app", "src", "main", "java", "br", "com", "comunidadesantaluzia", "app", "OfflineStorePlugin.java"))
for (const marker of ["DB_VERSION = 2", "setWriteAheadLoggingEnabled(true)", "PRAGMA integrity_check", "TABLE_BACKUPS", "recoverDocument"]) if (!offlineStore.includes(marker)) fail(`SQLite local sem marcador: ${marker}`)
...
if (!new RegExp(`versionCode\\s+${config.versionCode}`).test(gradleFinal)) fail("versionCode Beta 13 não aplicado.")
if (!gradleFinal.includes(`versionName "${config.versionName}"`)) fail("versionName Beta 13 não aplicado.")
...
console.log(`[motion-beta13] ${config.versionName}/code${config.versionCode}: pacote local, portas acionadas por login/logout, SQLite, identidade e assinatura persistente validados.`)
