...
const visualBurstName = "android-visual-burst-beta21.js"
...
const visualBurstFile = path.join(motionDir, visualBurstName)
if (!fs.existsSync(visualBurstFile) || fs.statSync(visualBurstFile).size < 3500) fail("Capturador visual de alta frequência Beta 21 ausente ou truncado.")
...
const visualBurstTag = `    <script defer src="/motion/${visualBurstName}"></script>`
...
if (!html.includes(visualBurstTag)) {
  if (html.includes(visualForensicsTag)) html = html.replace(visualForensicsTag, `${visualForensicsTag}\n${visualBurstTag}`)
  else fail("Ponto de inserção do capturador visual de alta frequência não encontrado.")
}
if (!html.includes(diagnosticLedgerTag)) {
  if (html.includes(visualBurstTag)) html = html.replace(visualBurstTag, `${visualBurstTag}\n${diagnosticLedgerTag}`)
...
if (scripts.length < 14) fail(`Stack Motion incompleta: somente ${scripts.length} script(s).`)
...
if (!scripts.includes(visualBurstName)) fail("Capturador visual de alta frequência Beta 21 não entrou na stack.")
...
if (scripts.indexOf(visualBurstName) !== scripts.indexOf(visualForensicsName) + 1) fail("Capturador de alta frequência deve carregar imediatamente depois do vigia forense.")
if (scripts.indexOf(diagnosticLedgerName) !== scripts.indexOf(visualBurstName) + 1) fail("Diagnostic Ledger deve carregar imediatamente depois do capturador visual.")
...
const bundle = `"use strict";\n/* Santa Luzia Motion Beta 21 — runtime consolidado com caixa-preta, Scroll Watchdog, vigia visual contínuo, burst de alta frequência, Diagnostic Ledger persistente e contadores isolados por execução. */\n${parts.join("\n")}`
...
if (!bundle.includes("santaLuziaVisualBurstBeta21") || !bundle.includes("precision-burst") || !bundle.includes("transition-run")) fail("Marcadores do capturador visual de alta frequência não foram consolidados.")
...
console.log(`[beta20-runtime] ${scripts.length} scripts consolidados em ${outputName} (${bundle.length} bytes), com vigia contínuo, captura de alta frequência e Diagnostic Ledger persistente.`)