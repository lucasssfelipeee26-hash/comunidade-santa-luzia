...
exigir(build.versionCode > release.versionCode || build.versionName === release.versionName, "Build pendente pode estar à frente da release publicada", `${build.versionName} code ${build.versionCode} / publicada ${release.versionName} code ${release.versionCode}`)
...
const progressoRotaNaoDestrutivo = process.env.SANTA_LUZIA_MOTION_BETA === "1" && existe("components/navigation-progress.tsx")
exigir(existe("app/loading.tsx") || progressoRotaNaoDestrutivo, "Aplicação possui estado global de carregamento ou progresso de rota não destrutivo")
