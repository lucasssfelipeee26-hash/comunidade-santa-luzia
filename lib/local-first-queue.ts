...
async function nativeStore(): Promise<NativeStoreHandle | null> {
  ...
  try {
    ...
    // O proxy retornado por registerPlugin expõe qualquer propriedade como método,
    // inclusive "then". Retorná-lo diretamente de uma função async faz o Promise
    // tratá-lo como thenable e o Capacitor tenta executar OfflineStore.then(), método
    // que não existe. Envolvemos o proxy em um objeto comum para impedir isso.
