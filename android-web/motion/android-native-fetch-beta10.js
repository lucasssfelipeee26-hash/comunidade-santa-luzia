...
(() => {
  ...
  function plugin() {
    return window.Capacitor?.Plugins?.SyncHttp || null;
  }
      ...
      for (const [name, value] of body.entries()) {
        if (value instanceof Blob) {
          entries.push({
            name,
            kind: "blob",
            filename: value instanceof File ? value.name : "arquivo",
  ...
  function responseFromNative(result) {
    ...
    const body = result?.bodyBase64
      ? base64ToBytes(String(result.bodyBase64))
      : String(result?.body ?? "");
  ...
  window.fetch = async function santaLuziaNativeSyncFetch(input, init) {
    ...
    const result = await native.request({
      ...
      method,
      headersJson: safeHeaders(input, init, request),
      ...descriptor,
    });
    if (init?.signal?.aborted || request?.signal?.aborted) throw new DOMException("The operation was aborted.", "AbortError");
