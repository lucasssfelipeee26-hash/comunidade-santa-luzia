...
export function LoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Login aceito: mostra a cena inteira do personagem caminhando de costas
      // em direção à porta, acenando, e só navega quando a cena termina.
      setEntering(true)
  ...
  return (
    <>
      <DoorTransitionScene direction="enter" active={entering} />
      <form onSubmit={handleSubmit} className="space-y-5" data-login-door-transition="true">
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <Input id="usuario" name="usuario" type="text" className="pl-9" placeholder="lucas.santos ou seu@email.com" autoCapitalize="none" autoCorrect="off" autoComplete="username" spellCheck={false} required />
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <Input id="senha" name="senha" type={mostrarSenha ? "text" : "password"} className="pl-9 pr-11" placeholder="Digite sua senha" autoComplete="current-password" required />
        ...
        <Button type="submit" className="w-full" disabled={loading || entering} aria-busy={loading || entering}>
          {loading && !entering ? <><Loader2 className="size-4 animate-spin" />Entrando…</> : <><ProfileDoorIcon className="size-5" animated={false} loop={false} direction="enter" /><span>{entering ? "Entrando…" : "Entrar"}</span></>}
        </Button>

        <div className="relative py-1"><div className="absolute inset-0 flex items-center"><span className="w-full border-t border-border" /></div><div className="relative flex justify-center text-[10px] font-semibold uppercase tracking-[.16em] text-muted-foreground"><span className="bg-white px-3">ou</span></div></div>

        <Link href="/visitante" className="flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-[#d4af37]/70 bg-[#fff8e7] px-4 py-3 text-sm font-bold text-[#6a1425] transition active:scale-[.99]"><BookOpenText className="size-4" />Continuar como visitante</Link>
        <p className="-mt-2 text-center text-[11px] leading-4 text-muted-foreground">Sem login: Liturgia Diária, Escala do Dia e Biblioteca.</p>

        <Link data-web-download-only href="/baixar" className="flex items-center justify-center gap-1.5 text-xs font-semibold text-primary hover:underline"><Download className="size-3.5" /> Baixar o aplicativo Android</Link>

        <p className="text-center text-sm text-muted-foreground">Ainda não tem acesso? <Link href="/area-restrita/cadastro" className="font-medium text-primary hover:underline">Cadastre-se</Link></p>
      </form>
    </>
  )
}
