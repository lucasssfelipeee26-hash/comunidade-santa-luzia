...
function MenuArea({ itens, rotulo }: { itens: ItemMenu[]; rotulo: string }) {
  ...
  return (
    <div className="relative shrink-0" data-no-pull-refresh>
      <button type="button" aria-label={rotulo} aria-expanded={aberto} onClick={() => setAberto((valor) => !valor)} className="inline-flex size-10 items-center justify-center rounded-2xl border border-white/70 bg-white/75 text-primary shadow-md backdrop-blur-xl transition active:scale-95">
        ...
        <>
          ...
          <nav role="dialog" aria-modal="true" aria-label="Menu da Área Restrita" className="app-nav-panel fixed left-1/2 top-1/2 z-[80] w-[calc(100%_-_24px)] max-w-[480px] -translate-x-1/2 -translate-y-1/2 rounded-[28px] border border-white/70 bg-white/90 p-4 shadow-2xl backdrop-blur-2xl">
            <div className="mb-3 flex items-center justify-between gap-2">
              <div className="min-w-0"><p className="text-[11px] font-bold uppercase tracking-[.14em] text-primary">Navegação</p><p className="text-xs text-muted-foreground">Escolha uma área</p></div>
            ...
            <div className="app-nav-grid grid grid-cols-3 gap-2 sm:grid-cols-4 sm:gap-3">
              {itens.map((item) => {
                ...
                return (
                  <Link prefetch={false} key={item.href} href={item.href} data-sl-nav-motion={item.motion} onClick={() => setAberto(false)} className={`app-nav-tile flex min-w-0 flex-col items-center gap-1.5 rounded-2xl px-1 py-2.5 text-center transition active:scale-95 ${ativo ? "bg-primary/10 text-primary ring-1 ring-primary/20" : "bg-white/65 text-foreground hover:bg-white"}`} title={item.label}>
                    <span className={`flex size-10 shrink-0 items-center justify-center rounded-2xl shadow-sm ${ativo ? "bg-primary text-white" : "bg-white text-primary"}`}>{item.icon}</span>
                    <span className="line-clamp-2 min-h-[2rem] max-w-full break-words text-[9px] font-bold leading-4">{item.curto}</span>
                  </Link>
                )
              })}
            </div>
