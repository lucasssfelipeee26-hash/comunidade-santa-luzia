...
type Submenu="ordinario"|"eucaristicas"|"prefacios"|"prefacios-proprios"|"prefacios-tempo"|null
...
const maisBase:Tela[]=[
 {id:"evangelho",titulo:"Evangelho e Lectio Divina",categoria:"evangelho",busca:""},{id:"lecionario",titulo:"Lecionário",categoria:"lecionario",busca:""},{id:"rosario",titulo:"Santo Rosário",categoria:"rosario"},{id:"salterio",titulo:"Saltério",categoria:"salterio",documento:"salterio/salterio.htm"},{id:"catequeses",titulo:"Catequeses",categoria:"catequeses",busca:""},{id:"comentarios",titulo:"Comentários",categoria:"comentarios",busca:""},{id:"oracoes",titulo:"Orações",categoria:"geral",busca:"oração"},{id:"indice",titulo:"Índice Geral",categoria:"oficio",busca:""},
]
...
export function CentralLiturgicaILiturgia(){
 ...
 const mais=useMemo<Tela[]>(()=>maisBase.map(item=>item.id==="rosario"?{...item,titulo:`Santo Rosário · ${misterioRosarioDoDia(agora)}`,documento:documentoRosarioDoDia(agora)}:item),[agora])
 function abrir(t:Tela){if(t.id==="ordinario"){setSubmenu("ordinario");return}if(t.id==="eucaristicas"){setSubmenu("eucaristicas");return}if(t.id==="prefacios"){setSubmenu("prefacios");return}if(t.id==="prefacios-proprios"){setSubmenu("prefacios-proprios");return}if(t.id==="prefacios-tempo"){setSubmenu("prefacios-tempo");return}setTela(t)}
 function voltarMissal(){if(submenu==="prefacios-proprios"||submenu==="prefacios-tempo")setSubmenu("prefacios");else setSubmenu(null)}
