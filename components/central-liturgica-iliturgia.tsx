...
export function CentralLiturgicaILiturgia(){
 const[modulo,setModulo]=useState<Modulo>("hoje"),[tela,setTela]=useState<Tela|null>(null)
 ...
 return <section className="relative min-h-[82vh] overflow-hidden rounded-2xl border border-[#745a3d]/30 bg-[#efe2bf] pb-20 shadow-sm">
  <header className="bg-[#62412d] px-4 py-4 text-[#fff5dc] shadow-sm"><div className="mx-auto flex max-w-4xl items-center justify-between gap-3"><div className="flex items-center gap-3"><Menu className="size-6"/><div><h1 className="font-serif text-2xl font-bold">Central Litúrgica</h1><p className="text-[11px] capitalize text-[#e9d6ad]">{dataCompleta}</p></div></div><Church className="size-7 text-[#f1d28a]"/></div></header>
  <div className="mx-auto max-w-4xl p-3 sm:p-5">
   {modulo==="hoje"&&<div className="space-y-4">
    {santo?<button type="button" onClick={()=>setTela({id:"santo-hoje",titulo:santo.nome,categoria:"oficio",documento:`oficio/proprio/oficiodasleituras/${santo.chave}.htm`})} className="grid w-full grid-cols-[110px_1fr] overflow-hidden rounded-xl border border-[#80694d]/35 bg-[#f8edcf] text-left shadow-sm sm:grid-cols-[150px_1fr]"><div className="relative min-h-[155px] bg-[#d8c49a]"><img src={santo.imagem} alt={santo.nome} className="absolute inset-0 size-full object-cover" onError={e=>{e.currentTarget.style.display="none"}}/></div><div className="flex flex-col justify-center p-4"><p className="text-[11px] font-bold uppercase tracking-[.18em] text-[#997026]">Santo do dia</p><h2 className="mt-1 font-serif text-2xl font-bold text-[#583b28] sm:text-3xl">{santo.nome}</h2>{santo.subtitulo&&<p className="mt-1 text-sm italic text-[#705c47]">{santo.subtitulo}</p>}<p className="mt-3 text-xs font-semibold text-[#7d5e37]">Abrir conteúdo do dia</p></div></button>:<div className="rounded-xl border border-[#80694d]/35 bg-[#f8edcf] p-4"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#997026]">Hoje</p><p className="mt-1 font-serif text-xl font-semibold text-[#583b28]">Calendário litúrgico do dia</p></div>}
    <button onClick={()=>setTela({id:"liturgia-hoje",titulo:"Liturgia diária",tipo:"liturgia"})} className="flex w-full items-center gap-4 rounded-xl border border-[#80694d]/35 bg-[#f8edcf] p-4 text-left shadow-sm"><span className="flex size-11 items-center justify-center rounded-full bg-[#68452f] text-[#f8e6b8]"><Church className="size-5"/></span><div><p className="text-[11px] font-bold uppercase tracking-[.16em] text-[#997026]">Liturgia do dia</p><h3 className="font-serif text-xl font-bold text-[#583b28]">Leituras e Evangelho</h3></div></button>
   </div>}
   {modulo==="oficio"&&<ListaModulo itens={horas} abrir={setTela}/>} 
   {modulo==="liturgia"&&<div className="space-y-3"><button onClick={()=>setTela({id:"liturgia",titulo:"Liturgia diária",tipo:"liturgia"})} className="flex w-full items-center gap-4 rounded-xl border border-[#80694d]/35 bg-[#f8edcf] p-4 text-left"><Church className="size-6 text-[#68452f]"/><span className="font-serif text-xl font-semibold italic text-[#583b28]">Liturgia diária</span></button><ListaModulo itens={[{id:"leituras",titulo:"Leituras",categoria:"lecionario",busca:""},{id:"evangelho",titulo:"Evangelho",categoria:"evangelho",busca:""},{id:"propria",titulo:"Leitura própria",categoria:"lecionario",busca:"Próprio"}]} abrir={setTela}/></div>}
   {modulo==="missal"&&<ListaModulo itens={missal} abrir={setTela}/>} 
   {modulo==="mais"&&<ListaModulo itens={mais} abrir={setTela}/>} 
  </div>
  <nav className="fixed inset-x-0 bottom-0 z-30 mx-auto grid max-w-[920px] grid-cols-5 border-t border-[#5b412d]/30 bg-[#62412d] text-[#e8d7b3] shadow-[0_-4px_12px_rgba(0,0,0,.12)] sm:rounded-t-2xl">{([['hoje','Hoje',Church],['oficio','Ofício',Clock3],['liturgia','Liturgia',BookOpenText],['missal','Missal',ScrollText],['mais','Mais',MoreHorizontal]] as const).map(([id,label,I])=><button key={id} onClick={()=>setModulo(id)} className={`flex flex-col items-center gap-1 px-1 py-2.5 text-[10px] font-semibold ${modulo===id?"bg-[#765239] text-[#fff1c9]":""}`}><I className="size-5"/><span>{label}</span></button>)}</nav>
 </section>
}
