...
export function FormacaoMembros() {
  ...
  const itens = useMemo(() => {
    ...
    return resposta.formacoes.map((formacao) => {
      // Marcações antigas criadas antes desta correção não podem fazer uma
      // formação futura parecer já respondida. O servidor também rejeita a fila
      // quando a sincronização ocorrer fora do dia correto.
    ...
    <div className="space-y-8">
      {!online && (
        <p className="flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          ...
          Você está sem internet. As formações já sincronizadas continuam disponíveis; no dia da formação, sua presença pode ficar salva para envio automático quando a conexão voltar.
        </p>
      )}

      <section>
        <h2 className="mb-4 flex items-center gap-2 font-serif text-3xl text-[#0b4b35]">
          <CalendarDays className="size-6 text-[#9a731d]" /> Próxima formação
        </h2>
        {!proxima ? (
          <p className="rounded-xl border bg-white p-5 text-muted-foreground">
            Nenhuma próxima formação publicada no momento.
          </p>
        ) : (
          <div className="max-w-3xl">
            <CardProxima
              ...
              onAtualizada={(presenca) => atualizarMinhaPresenca(proxima.id, presenca)}
            />
          </div>
        )}
      </section>
      ...
      <section>
        <h2 className="mb-4 flex items-center gap-2 font-serif text-2xl text-[#0b4b35]">
          <History className="size-5 text-[#9a731d]" /> Histórico de formações
        </h2>
        {historico.length === 0 ? (
          <p className="rounded-xl border border-dashed bg-white p-5 text-sm text-muted-foreground">
            As formações realizadas aparecerão aqui automaticamente depois da data.
          </p>
        ) : (
          <div className="grid gap-4 md:grid-cols-2">
            {historico.map((item) => <CardHistorico key={item.id} item={item} />)}
          </div>
        )}
      </section>
    </div>
  ...
  return (
    <>
      <div className="flex items-start justify-between gap-3">
        ...
        <span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-bold ${item.status === "cancelada" ? "bg-destructive/10 text-destructive" : "bg-[#073b29]/10 text-[#073b29]"}`}>
...
function MaterialFormacao({ item }: { item: FormacaoComPresenca }) {
  ...
  return (
    <a href={`/api/formacoes/${item.id}/download`} className="mt-4 inline-flex max-w-full items-center gap-2 rounded-md border border-[#d4af37] bg-[#fffaf0] px-4 py-2.5 text-sm font-semibold text-[#755611] hover:bg-[#d4af37] hover:text-[#073b29]">
      ...
      <span className="truncate">Baixar {item.arquivo.nome_original}</span>
      <span className="shrink-0 text-xs opacity-70">({tamanho(item.arquivo.tamanho)})</span>
    </a>
  )
}
...
function CardProxima({
  ...
  return (
    <article className={`rounded-xl border bg-white p-5 shadow-sm ${item.status === "cancelada" ? "border-destructive/40" : "border-[#d4af37]/35"}`}>
      ...
      {item.status !== "cancelada" && !ehHoje && (
        <div className="mt-4 flex items-start gap-2 rounded-2xl border border-[#d4af37]/35 bg-[#fff8e6] p-3 text-sm leading-5 text-[#6f541a]">
          <LockKeyhole className="mt-0.5 size-4 shrink-0" />
          <span><strong>Presença bloqueada por enquanto.</strong> Os botões serão liberados somente no dia {formatarData(item.data)}.</span>
        </div>
      )}
  ...
  return (
    <article className="rounded-xl border border-[#d4af37]/30 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="text-[11px] font-bold uppercase tracking-[.1em] text-[#9a731d]">{formatarData(item.data)}{item.horario ? ` · ${item.horario}` : ""}</p>
          <h3 className="mt-1 font-serif text-xl text-[#123f2e]">{item.titulo}</h3>
          <p className="mt-1 text-sm text-[#6d6255]">Tema: {item.tema}</p>
        </div>
        {item.status === "cancelada" ? (
          <span className="rounded-full border border-red-200 bg-red-50 px-3 py-1 text-xs font-bold text-red-800">Cancelada</span>
        ) : situacao ? (
          <span className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-bold ${situacao.classe}`}>{situacao.icone}{situacao.rotulo}</span>
        ) : (
          <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-600">Presença não registrada</span>
        )}
      </div>
      ...
      {item.minha_presenca?.status === "justificada" && item.minha_presenca.justificativa && (
        <p className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><strong>Justificativa:</strong> {item.minha_presenca.justificativa}</p>
      )}
      <MaterialFormacao item={item} />
    </article>
  )
...
function MinhaPresencaControle({
  ...
  async function salvar() {
    if (dataFormacao !== hojeCuiaba()) {
      setMensagem({ tipo: "erro", texto: "A presença só pode ser marcada no dia da formação." })
      return
    }
    if (!situacao) {
  ...
  return (
    <div data-no-pull-refresh className="mt-4 rounded-2xl border border-[#e2d8d2] bg-[#fffaf7] p-3.5">
      ...
      {situacao === "justificada" && (
        <div className="mt-3">
          <label htmlFor={`minha-justificativa-${formacaoId}`} className="mb-1.5 block text-xs font-semibold text-[#5f5658]">Justificativa</label>
          <textarea
      ...
      {presenca?.pendente && (
        <p className="mt-2 flex items-center gap-2 text-xs font-semibold text-sky-800"><Clock3 className="size-4" /> Pendente de sincronização.</p>
      )}
      {mensagem && (
