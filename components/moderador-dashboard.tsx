    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto w-full max-w-6xl overflow-x-hidden px-3 py-4 sm:px-4 sm:py-8">
        ...
        <div className="mb-6 grid gap-3 sm:mb-8 sm:grid-cols-2">
          <Link
            href="/area-restrita/atrasos"
            className="flex items-center gap-3 rounded-2xl border border-amber-300/60 bg-amber-50 p-4 transition hover:border-amber-400 hover:bg-amber-100/70"
          >
            <span className="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-amber-600 text-white">
              <Clock className="size-5" aria-hidden="true" />
            </span>
            <span className="min-w-0 flex-1">
              <strong className="block text-amber-950">Atrasos</strong>
              <span className="mt-0.5 block text-sm text-amber-900/75">Confira, confirme ou rejeite os relatos enviados pelos membros.</span>
            </span>
            <ChevronRight className="size-5 shrink-0 text-amber-800" aria-hidden="true" />
          </Link>

          <Link
            href="/area-restrita/moderador/presencas"
            className="flex items-center gap-3 rounded-2xl border border-primary/25 bg-primary/5 p-4 transition hover:border-primary/45 hover:bg-primary/10"
          >
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="mt-0.5 block text-sm text-muted-foreground">Consulte presenças, faltas, justificativas e o histórico de toda a equipe.</span>
            </span>
        ...
        {pendentes.length > 0 && (
          <section className="mb-8">
            ...
            <ul className="space-y-3">
              {pendentes.map((m) => (
                <li key={m.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-accent/45 bg-white p-4 shadow-sm">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="break-words text-xs leading-5 text-muted-foreground sm:text-sm">{m.funcao} · usuário: {m.usuario} · e-mail: {m.email}</p>
                  </div>
                  <div className="flex gap-2">
                    <Button size="sm" onClick={() => aprovarMembro(m.id)} className="gap-1.5">
                      <UserCheck className="size-4" aria-hidden="true" /> Aprovar
                    </Button>
                    <Button size="sm" variant="outline" onClick={() => recusarMembro(m.id)} className="gap-1.5 border-destructive/40 text-destructive hover:bg-destructive/10">
                      <UserX className="size-4" aria-hidden="true" /> Recusar
                    </Button>
                  </div>
                </li>
              ))}
        ...
        <ul className="grid gap-4 sm:grid-cols-2">
          {ativos.map((m) => <li key={m.id}><PerfilResumo membro={m} /></li>)}
        </ul>
      </main>
    </div>
  )
...
function PerfilResumo({ membro: m }: { membro: Membro }) {
  return (
    <Link href={`/area-restrita/perfil/${m.id}`} className="group flex items-center gap-4 rounded-xl border border-border bg-card p-4 transition-all hover:border-primary/40 hover:shadow-md">
      <Avatar className="size-12 border border-border">
        <AvatarImage src={m.foto || undefined} /><AvatarFallback className="bg-primary/10 font-medium text-primary">{iniciais(m.nome)}</AvatarFallback>
      </Avatar>
      <div className="min-w-0 flex-1">
        <p className="truncate font-medium text-foreground">{m.nome}</p>
        <p className="text-sm text-muted-foreground">{m.funcao} · desde {m.desde || "data não informada"}</p>
        <div className="mt-2 flex flex-wrap gap-2 text-xs">
          {m.advertencias.length > 0 && <span className="inline-flex items-center gap-1 rounded-full bg-destructive/10 px-2 py-0.5 text-destructive"><TriangleAlert className="size-3" aria-hidden="true" />{m.advertencias.length} advert.</span>}
          {m.faltas.length > 0 && <span className="inline-flex items-center gap-1 rounded-full bg-secondary px-2 py-0.5 text-secondary-foreground"><CalendarX className="size-3" aria-hidden="true" />{m.faltas.length} faltas</span>}
          {m.justificativas.length > 0 && <span className="inline-flex items-center gap-1 rounded-full bg-primary/10 px-2 py-0.5 text-primary"><FileText className="size-3" aria-hidden="true" />{m.justificativas.length} justif.</span>}
          {m.advertencias.length === 0 && m.faltas.length === 0 && <span className="rounded-full bg-[oklch(0.6_0.08_160)]/15 px-2 py-0.5 text-[oklch(0.45_0.08_160)]">Em dia</span>}
        </div>
      </div>
