...
import { BookOpenText, CheckCircle2, Download, HardDriveDownload, Library, Loader2, Search, X } from "lucide-react"
...
async function descompactarJson(resposta: Response): Promise<Pacote> {
  ...
  if (!("DecompressionStream" in window)) {
    throw new Error("Este navegador não suporta a descompactação offline do acervo.")
  }
...
export function AcervoLiturgicoOffline() {
  ...
  useEffect(() => {
    fetch(`${BASE}/manifest.json`, { cache: "no-store" })
      .then(async (r) => {
        if (!r.ok) throw new Error("Manifesto do acervo ainda não foi publicado.")
        const m = await r.json() as Manifesto
        setManifesto(m)
        if (m.categorias.length) setCategoria(m.categorias[0].id)
        try {
          const cache = await caches.open(CACHE)
          const urls = m.categorias.flatMap((c) => c.arquivos.map((a) => `${BASE}/${a}`))
  ...
  useEffect(() => {
    ...
    setCarregando(true)
    setErro("")
    Promise.all(cat.arquivos.map(async (arquivo) => {
      const resposta = await obterResposta(`${BASE}/${arquivo}`)
      return descompactarJson(resposta)
    }))
      .then((pacotes) => {
        if (!cancelado) setDocumentos(pacotes.flatMap((p) => p.documents))
      })
  ...
  if (aberto) {
    return (
      <article className="rounded-3xl border border-[#d4af37]/35 bg-white/90 p-4 shadow-sm sm:p-6">
