...
type Props={categoriaInicial?:string;buscaInicial?:string;embutido?:boolean;titulo?:string}
...
function normalizar(v:string){
  return v.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().replace(/santa/g,"sta").replace(/santo/g,"sto").replace(/sao/g,"sao").replace(/[^a-z0-9]/g,"")
}
function candidatosImagem(d:Documento){
  const bases=[normalizar(d.title),normalizar(d.path.split("/").pop()||"")].filter(Boolean)
  const unicos=[...new Set(bases)]
  return unicos.flatMap(b=>[`/${b}.jpg`,`/${b}.jpeg`,`/${b}.png`,`/${b}.webp`])
}

function ImagemDocumento({doc}:{doc:Documento}){
  const urls=useMemo(()=>candidatosImagem(doc),[doc])
  const [idx,setIdx]=useState(0)
  if(idx>=urls.length)return null
  return <img src={urls[idx]} alt="" onError={()=>setIdx(v=>v+1)} className="mx-auto mt-4 max-h-[420px] w-auto max-w-full rounded-2xl border border-[#d4af37]/35 object-contain shadow-sm" />
}

export function AcervoLiturgicoOffline({categoriaInicial="",buscaInicial="",embutido=false,titulo="Central Litúrgica"}:Props){
  ...
  useEffect(()=>{if(categoriaInicial)setCat(categoriaInicial);setBusca(buscaInicial);setAberto(null)},[categoriaInicial,buscaInicial])
  useEffect(()=>{if(!m||!cat)return;const c=m.categorias.find(x=>x.id===cat);if(!c)return;let off=false;setLoading(true);setErro("");Promise.all(c.arquivos.map(async a=>abrir(await fetch(pacoteUrl(a),{cache:"force-cache"})))).then(p=>{if(!off)setDocs(p.flatMap(x=>x.documents))}).catch(e=>{if(!off)setErro(e.message)}).finally(()=>{if(!off)setLoading(false)});return()=>{off=true}},[m,cat])
  ...
  if(aberto)return <article className="rounded-3xl border border-[#d4af37]/35 bg-[#fffdf8] p-4 shadow-sm sm:p-6">
    <button onClick={()=>setAberto(null)} className="mb-4 inline-flex items-center gap-2 rounded-xl border bg-white px-3 py-2 text-sm font-semibold"><X className="size-4"/>Voltar</button>
  ...
  return <div>
    {!embutido&&<div className="rounded-3xl border border-[#d4af37]/35 bg-white/85 p-4 sm:p-5"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#9a731d]">Conteúdo incorporado ao aplicativo</p><h2 className="mt-1 font-serif text-3xl font-semibold text-[#8f182e]">{titulo}</h2><p className="mt-2 text-sm text-muted-foreground">Textos, rubricas, símbolos e imagens litúrgicas incorporados à Central.</p></div>}
    ...
    {m&&<div className={`mt-4 grid gap-4 ${embutido?"":"lg:grid-cols-[270px_1fr]"}`}>
      ...
      <section className="rounded-2xl border bg-white/80 p-3 sm:p-4">
        <label className="flex items-center gap-2 rounded-xl border bg-white px-3"><Search className="size-4"/><input value={busca} onChange={e=>setBusca(e.target.value)} placeholder="Pesquisar oração, prefácio, leitura, santo ou referência…" className="h-12 w-full bg-transparent text-sm outline-none"/></label>
