...
import { CheckCircle2, Database, RefreshCw, Search, Trash2, Trophy, WifiOff } from "lucide-react"
...
export function AdministracaoModerador() {
  ...
  async function excluirCadastro() {
    ...
    setSucesso(`Cadastro de ${nome} excluído do banco com sucesso.`)
  ...
  async function zerarRanking() {
    ...
    const ano = dados.ano
    const json = await executar({ action: "resetar_ranking", ano, confirmacao: confirmacaoRanking }, "ranking")
    if (!json) return
    setDados((atual) => atual ? { ...atual, ranking: Array.isArray(json.ranking) ? json.ranking : atual.ranking } : atual)
    setConfirmacaoRanking("")
    setSucesso(`Placar de ${ano} resetado com sucesso. O histórico foi preservado.`)
    window.dispatchEvent(new Event("santa-luzia:server-sync"))
  }

  return (
    <details open className="group mb-3 overflow-hidden rounded-2xl border border-primary/15 bg-white/85 shadow-sm" data-admin-database-tools="true" data-admin-default-open="true">
      ...
      <div className="border-t border-primary/10 p-3">
        ...
        {sucesso && <div role="status" className="mb-3 flex items-start gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-[10px] leading-4 text-emerald-900"><CheckCircle2 className="mt-0.5 size-3.5 shrink-0" />{sucesso}</div>}
        ...
        <section className="rounded-2xl border border-destructive/15 bg-destructive/[.025] p-3">
          ...
          <div className="mt-2 max-h-48 space-y-1.5 overflow-y-auto overscroll-contain pr-1" data-no-pull-refresh data-admin-member-list="true">
            {filtrados.map((cadastro) => (
              <button key={cadastro.id} type="button" onClick={() => { setAlvo(cadastro); setConfirmacaoExcluir(""); setSucesso("") }} className={`flex w-full min-w-0 items-center gap-2 rounded-xl border p-2 text-left transition ${alvo?.id === cadastro.id ? "border-destructive/45 bg-destructive/5" : "border-border bg-white"}`} data-admin-member-select={cadastro.id}>
                <Avatar className="size-8 shrink-0"><AvatarImage src={cadastro.foto || undefined} className="h-full w-full object-cover object-center" /><AvatarFallback className="text-[9px]">{iniciais(cadastro.nome)}</AvatarFallback></Avatar>
            ...
            {!carregando && dados && filtrados.length === 0 && <p className="rounded-xl border border-dashed p-3 text-center text-[10px] text-muted-foreground">Nenhum cadastro encontrado.</p>}
          </div>
          {alvo && <div className="mt-3 rounded-xl border border-destructive/20 bg-white p-3" data-admin-delete-confirm="true"><p className="text-[10px] leading-4">Você selecionou <strong>{alvo.nome}</strong>. Digite <strong>EXCLUIR</strong> para confirmar.</p><div className="mt-2 flex gap-2"><Input value={confirmacaoExcluir} onChange={(event) => setConfirmacaoExcluir(event.target.value)} placeholder="EXCLUIR" className="h-9 text-[10px]" autoComplete="off" /><Button type="button" variant="destructive" className="h-9 shrink-0 text-[10px]" onClick={() => void excluirCadastro()} disabled={processando !== null || confirmacaoExcluir.trim().toUpperCase() !== "EXCLUIR"}>{processando === "excluir" ? "Excluindo…" : "Excluir"}</Button></div></div>}
        </section>

        <section className="mt-3 rounded-2xl border border-[#d4af37]/35 bg-[#fff9e9] p-3" data-admin-ranking-reset="true">
