...
application = (NATIVE.parent.parent.parent.parent.parent.parent.parent / "SantaLuziaApplication.kt") if False else (NATIVE / "SantaLuziaApplication.kt")
...
require('database.enqueue' in repository and 'mutation_queue' in (NATIVE / "core" / "data" / "NativeDatabase.kt").read_text(encoding="utf-8"), "Resposta offline não usa fila persistente")
require('Regex("^/api/quizzes/[^/]+/responder$")' in sync, "SyncWorker não reconhece replay de quiz avulso")

# 3) O Quiz Litúrgico cronometrado continua sendo uma regra diferente.
...
# 4) Criar/editar/publicar/excluir Quiz avulso é administrativo e necessariamente online.
...
# 5) Reconexão precisa ser idempotente: se o servidor já recebeu, não duplica pontuação.
...
# 6) O mesmo ciclo que baixa o Quiz também atualiza notificações; leitura offline é persistida.
