...
internal class SantaLuziaRepository(
    ...
    suspend fun login(login: String, password: String): RepositoryResult<NativeSession> {
        val payload = JSONObject().put("usuario", login.trim()).put("senha", password).toString()
        return try {
            ...
            if (!response.successful || !json.optBoolean("ok")) {
                RepositoryResult.Failure(json.optString("erro").ifBlank { "Não foi possível entrar." }, response.status)
            } else {
    ...
    suspend fun deleteOwnAccount(password: String, confirmation: String): RepositoryResult<Unit> = withContext(Dispatchers.IO) {
        if (confirmation.trim().uppercase() != "EXCLUIR") return@withContext RepositoryResult.Failure("Digite EXCLUIR para confirmar.")
        if (password.isBlank()) return@withContext RepositoryResult.Failure("Informe sua senha atual.")
        try {
            val payload = JSONObject().put("senha", password).put("confirmacao", "EXCLUIR").toString()
            ...
            if (!response.successful || !json.optBoolean("ok")) {
                RepositoryResult.Failure(json.optString("erro").ifBlank { "Não foi possível excluir a conta." }, response.status)
            } else {
    ...
    suspend fun readLocalFirst(cacheKey: String, path: String, authenticated: Boolean = false): RepositoryResult<String> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (response.successful && response.body.isNotBlank()) {
                ...
                database.getDocument(resolvedCacheKey)?.let { RepositoryResult.Success(it.payload, fromCache = true) }
    ...
    suspend fun mutate(method: String, path: String, payload: String?, optimisticCacheKey: String? = null, optimisticPayload: String? = null): RepositoryResult<String> =
        mutateLocalFirst(method, path, payload, optimisticCacheKey, optimisticPayload)

    suspend fun mutateOnlineOnly(method: String, path: String, payload: String?): RepositoryResult<String> = withContext(Dispatchers.IO) {
        try {
            val response = http.request(method, path, payload)
            if (response.successful) RepositoryResult.Success(response.body)
            else {
                val serverMessage = runCatching { JSONObject(response.body).optString("erro") }.getOrNull()?.takeIf { it.isNotBlank() }
                RepositoryResult.Failure(serverMessage ?: "A alteração foi rejeitada pelo servidor (${response.status}).", response.status)
            }
        } catch (_: IOException) {
    ...
    suspend fun mutateMultipartOnlineOnly(
        ...
        try {
            val upload = if (fileField != null && fileName != null && fileBytes != null) MultipartUpload(fileField, fileName, mimeType.orEmpty(), fileBytes) else null
            val response = http.requestMultipart(method, path, fields, upload)
            if (response.successful) RepositoryResult.Success(response.body)
            else {
                val serverMessage = runCatching { JSONObject(response.body).optString("erro") }.getOrNull()?.takeIf { it.isNotBlank() }
                RepositoryResult.Failure(serverMessage ?: "O envio foi rejeitado pelo servidor (${response.status}).", response.status)
            }
        } catch (_: IOException) {
    ...
    suspend fun mutateLocalFirst(
        ...
        fun commitOptimisticCache() {
            if (optimisticResolvedKey != null && optimisticPayload != null) database.putDocument(optimisticResolvedKey, optimisticPayload)
        }
        try {
            val response = http.request(method, path, payload)
            when {
                ...
                mayQueue && (response.status in 500..599 || response.status == 408 || response.status == 429 || (preserveOnAuthFailure && response.status in setOf(401, 403))) -> {
                    ...
                    val message = runCatching { JSONObject(response.body).optString("erro") }.getOrNull()?.takeIf { it.isNotBlank() }
                        ...
                        } else "A alteração foi rejeitada pelo servidor (${response.status})."
            ...
            if (mayQueue) {
                ...
                commitOptimisticCache()
                RepositoryResult.Queued(id)
            } else RepositoryResult.Failure("Esta operação precisa de internet para evitar envio duplicado. Reconecte e tente novamente.")
        } catch (error: Exception) {
    ...
    private fun shouldPreserveOnAuthFailure(method: String, path: String, payload: String?): Boolean {
        ...
        if (verb == "POST" && Regex("^/api/quizzes/[^/]+/responder$").matches(path)) {
            ...
            return body.optString("clientRequestId").isNotBlank() && (body.optJSONArray("respostas")?.length() ?: 0) > 0
        }
    ...
    private fun canQueueOffline(method: String, path: String, payload: String?): Boolean {
        ...
        // Allowlist: qualquer mutação nova começa ONLINE-ONLY até existir prova de replay seguro.
        if (verb == "POST" && path == "/api/escalas") {
            ...
            return Regex("^[A-Za-z0-9._:-]{8,120}$").matches(requestId) &&
                ...
                celebrant.length in 2..120 && people != null && people.length() <= 80
        }

        // Formação sem anexo usa JSON e a mesma chave idempotente persistida no servidor.
        // Upload multipart continua fora da fila genérica para não armazenar binários no SQLite.
        if (verb == "POST" && path == "/api/formacoes") {
            val requestId = body?.optString("clientRequestId").orEmpty()
            val title = body?.optString("titulo").orEmpty().trim()
            val theme = body?.optString("tema").orEmpty().trim()
            val date = body?.optString("data").orEmpty()
            val time = body?.optString("horario").orEmpty()
            val description = body?.optString("descricao").orEmpty()
            return Regex("^[A-Za-z0-9._:-]{8,120}$").matches(requestId) &&
                title.length in 3..180 && theme.length in 3..180 &&
                Regex("^\\d{4}-\\d{2}-\\d{2}$").matches(date) &&
                (time.isBlank() || Regex("^(?:[01]\\d|2[0-3]):[0-5]\\d$").matches(time)) &&
        ...
        if (verb == "PATCH" && path == "/api/perfil") return body != null && body.length() > 0
        ...
        if (verb == "POST" && path == "/api/constancia-luz") return Regex("^\\d{4}-\\d{2}-\\d{2}$").matches(body?.optString("data").orEmpty())
        ...
        if (verb == "POST" && path == "/api/quizzes/liturgia/offline") {
            ...
            val requestId = body?.optString("clientRequestId").orEmpty()
            val answers = body?.optJSONArray("respostas")
            return Regex("^\\d{4}-\\d{2}-\\d{2}$").matches(dateIso) && requestId.isNotBlank() && answers != null && answers.length() > 0
        }
        if (verb == "POST" && path == "/api/ranking") {
            return body?.optString("action") == "reportar_atraso" && body.optString("clientRequestId").isNotBlank()
        }
        return false
    }
    ...
    suspend fun warmEssentialCaches() {
        ...
        essentials.forEach { (key, path) -> readLocalFirst(cacheKey = key, path = path, authenticated = key != "biblioteca") }
    }
}
